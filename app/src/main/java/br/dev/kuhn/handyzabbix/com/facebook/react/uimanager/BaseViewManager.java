...
public abstract class BaseViewManager<T extends View, C extends LayoutShadowNode> extends ViewManager<T, C> implements BaseViewManagerInterface<T> {
    ...
    static {
        ...
        hashMap.put(STATE_BUSY, Integer.valueOf(C0873R.string.state_busy_description));
        hashMap.put(STATE_EXPANDED, Integer.valueOf(C0873R.string.state_expanded_description));
        hashMap.put("collapsed", Integer.valueOf(C0873R.string.state_collapsed_description));
    }
    ...
    public void setAccessibilityRole(T t, String str) {
        if (str != null) {
            t.setTag(C0873R.C0876id.accessibility_role, ReactAccessibilityDelegate.AccessibilityRole.fromValue(str));
        }
    }
    ...
    public void setViewState(T t, ReadableMap readableMap) {
        if (readableMap != null) {
            t.setTag(C0873R.C0876id.accessibility_state, readableMap);
            t.setSelected(false);
            t.setEnabled(true);
    ...
    private void updateViewContentDescription(T t) {
        Dynamic dynamic;
        String str = (String) t.getTag(C0873R.C0876id.accessibility_label);
        ReadableMap readableMap = (ReadableMap) t.getTag(C0873R.C0876id.accessibility_state);
        String str2 = (String) t.getTag(C0873R.C0876id.accessibility_hint);
        ArrayList arrayList = new ArrayList();
        ReadableMap readableMap2 = (ReadableMap) t.getTag(C0873R.C0876id.accessibility_value);
        if (str != null) {
            arrayList.add(str);
        }
        if (readableMap != null) {
            ...
            while (keySetIterator.hasNextKey()) {
                ...
                if (nextKey.equals(STATE_CHECKED) && dynamic2.getType() == ReadableType.String && dynamic2.asString().equals(STATE_MIXED)) {
                    arrayList.add(t.getContext().getString(C0873R.string.state_mixed_description));
                } else if (nextKey.equals(STATE_BUSY) && dynamic2.getType() == ReadableType.Boolean && dynamic2.asBoolean()) {
                    arrayList.add(t.getContext().getString(C0873R.string.state_busy_description));
                } else if (nextKey.equals(STATE_EXPANDED) && dynamic2.getType() == ReadableType.Boolean) {
                    arrayList.add(t.getContext().getString(dynamic2.asBoolean() ? C0873R.string.state_expanded_description : C0873R.string.state_collapsed_description));
                }
            }
        }
    ...
    public void setAccessibilityActions(T t, ReadableArray readableArray) {
        if (readableArray != null) {
            t.setTag(C0873R.C0876id.accessibility_actions, readableArray);
        }
    }
    ...
    public void setAccessibilityValue(T t, ReadableMap readableMap) {
        if (readableMap != null) {
            t.setTag(C0873R.C0876id.accessibility_value, readableMap);
    ...
    public Map<String, Object> getExportedCustomDirectEventTypeConstants() {
        return MapBuilder.builder().put(ReactAccessibilityDelegate.TOP_ACCESSIBILITY_ACTION_EVENT, MapBuilder.m123of("registrationName", "onAccessibilityAction")).build();
    }
    ...
    private void logUnsupportedPropertyWarning(String str) {
        FLog.m90w(ReactConstants.TAG, "%s doesn't support property '%s'", getName(), str);
    }
}
