...
class SpringAnimation extends AnimationDriver {
    ...
    public void resetConfig(ReadableMap readableMap) {
        ...
        this.mHasFinished = z;
        this.mCurrentLoop = 0;
        this.mTimeAccumulator = Utils.DOUBLE_EPSILON;
    ...
    private boolean isAtRest() {
        return Math.abs(this.mCurrentState.velocity) <= this.mRestSpeedThreshold && (getDisplacementDistanceForState(this.mCurrentState) <= this.mDisplacementFromRestThreshold || this.mSpringStiffness == Utils.DOUBLE_EPSILON);
    }

    private boolean isOvershooting() {
        return this.mSpringStiffness > Utils.DOUBLE_EPSILON && ((this.mStartValue < this.mEndValue && this.mCurrentState.position > this.mEndValue) || (this.mStartValue > this.mEndValue && this.mCurrentState.position < this.mEndValue));
    }
    ...
    private void advance(double d) {
        ...
        if (!isAtRest()) {
            ...
            if (isAtRest() || (this.mOvershootClampingEnabled && isOvershooting())) {
                if (this.mSpringStiffness > Utils.DOUBLE_EPSILON) {
