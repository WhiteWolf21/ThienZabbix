...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    public ClockFaceView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, C1242R.attr.materialClockStyle);
    }
    ...
    public ClockFaceView(Context context, AttributeSet attributeSet, int i) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C1242R.styleable.ClockFaceView, i, C1242R.style.Widget_MaterialComponents_TimePicker_Clock);
        Resources resources = getResources();
        ColorStateList colorStateList = MaterialResources.getColorStateList(context, obtainStyledAttributes, C1242R.styleable.ClockFaceView_clockNumberTextColor);
        this.textColor = colorStateList;
        LayoutInflater.from(context).inflate(C1242R.layout.material_clockface_view, this, true);
        ClockHandView clockHandView2 = (ClockHandView) findViewById(C1242R.C1246id.material_clock_hand);
        this.clockHandView = clockHandView2;
        this.clockHandPadding = resources.getDimensionPixelSize(C1242R.dimen.material_clock_hand_padding);
        ...
        int defaultColor = AppCompatResources.getColorStateList(context, C1242R.C1244color.material_timepicker_clockface).getDefaultColor();
        ColorStateList colorStateList2 = MaterialResources.getColorStateList(context, obtainStyledAttributes, C1242R.styleable.ClockFaceView_clockFaceBackgroundColor);
        ...
        this.valueAccessibilityDelegate = new AccessibilityDelegateCompat() {
            public void onInitializeAccessibilityNodeInfo(View view, AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
                ...
                int intValue = ((Integer) view.getTag(C1242R.C1246id.material_value_index)).intValue();
                if (intValue > 0) {
        ...
        Arrays.fill(strArr, "");
        setValues(strArr, 0);
        this.minimumHeight = resources.getDimensionPixelSize(C1242R.dimen.material_time_picker_minimum_screen_height);
        this.minimumWidth = resources.getDimensionPixelSize(C1242R.dimen.material_time_picker_minimum_screen_width);
        this.clockSize = resources.getDimensionPixelSize(C1242R.dimen.material_clock_size);
    }
    ...
    private void updateTextViews(int i) {
        ...
        for (int i2 = 0; i2 < Math.max(this.values.length, size); i2++) {
            ...
            if (i2 >= this.values.length) {
                ...
                if (textView == null) {
                    textView = (TextView) from.inflate(C1242R.layout.material_clockface_textview, this, false);
                    this.textViewPool.put(i2, textView);
                    addView(textView);
                }
                textView.setVisibility(0);
                textView.setText(this.values[i2]);
                textView.setTag(C1242R.C1246id.material_value_index, Integer.valueOf(i2));
