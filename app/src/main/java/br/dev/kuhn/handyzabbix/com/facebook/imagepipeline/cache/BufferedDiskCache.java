...
public class BufferedDiskCache {
                ...
                public EncodedImage call() throws Exception {
                    ...
                    try {
                        ...
                        if (!atomicBoolean.get()) {
                            ...
                            if (encodedImage != null) {
                                FLog.m69v((Class<?>) BufferedDiskCache.TAG, "Found image for %s in staging area", (Object) cacheKey.getUriString());
                                BufferedDiskCache.this.mImageCacheStatsTracker.onStagingAreaHit(cacheKey);
                            } else {
                                FLog.m69v((Class<?>) BufferedDiskCache.TAG, "Did not find image for %s in staging area", (Object) cacheKey.getUriString());
                                ...
                                try {
                                    ...
                                    of = CloseableReference.m112of(access$400);
                            ...
                            FLog.m68v((Class<?>) BufferedDiskCache.TAG, "Host thread was interrupted, decreasing reference count");
                            if (encodedImage != null) {
                                encodedImage.close();
                            }
    ...
    public PooledByteBuffer readFromDiskCache(CacheKey cacheKey) throws IOException {
        ...
        try {
            Class<?> cls = TAG;
            FLog.m69v(cls, "Disk cache read for %s", (Object) cacheKey.getUriString());
            BinaryResource resource = this.mFileCache.getResource(cacheKey);
            if (resource == null) {
                FLog.m69v(cls, "Disk cache miss for %s", (Object) cacheKey.getUriString());
                this.mImageCacheStatsTracker.onDiskCacheMiss();
                return null;
            }
            FLog.m69v(cls, "Found entry in disk cache for %s", (Object) cacheKey.getUriString());
            ...
            openStream.close();
            FLog.m69v(cls, "Successful read from disk cache for %s", (Object) cacheKey.getUriString());
    ...
    public void writeToDiskCache(CacheKey cacheKey, final EncodedImage encodedImage) {
        Class<?> cls = TAG;
        FLog.m69v(cls, "About to write to disk-cache for key %s", (Object) cacheKey.getUriString());
        try {
            ...
            FLog.m69v(cls, "Successful disk-cache write for key %s", (Object) cacheKey.getUriString());
        } catch (IOException e) {
