...
abstract class AbstractLayoutAnimation {
    private static final Map<InterpolatorType, BaseInterpolator> INTERPOLATOR = MapBuilder.m126of(InterpolatorType.LINEAR, new LinearInterpolator(), InterpolatorType.EASE_IN, new AccelerateInterpolator(), InterpolatorType.EASE_OUT, new DecelerateInterpolator(), InterpolatorType.EASE_IN_EASE_OUT, new AccelerateDecelerateInterpolator());
    ...
    public void initializeFromConfig(ReadableMap readableMap, int i) {
        ...
        if (readableMap.hasKey(TypedValues.Transition.S_DURATION)) {
            i = readableMap.getInt(TypedValues.Transition.S_DURATION);
        }
        this.mDurationMs = i;
        this.mDelayMs = readableMap.hasKey("delay") ? readableMap.getInt("delay") : 0;
        if (readableMap.hasKey(NotificationsService.EVENT_TYPE_KEY)) {
            this.mInterpolator = getInterpolator(InterpolatorType.fromString(readableMap.getString(NotificationsService.EVENT_TYPE_KEY)), readableMap);
            if (!isValid()) {
